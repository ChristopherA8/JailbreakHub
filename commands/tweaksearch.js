...
module.exports = {
	data: new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('query').setDescription('Name of tweak').setRequired(true))
	...
	async execute(interaction) {
        ...
        if (data.status === false) return interaction.reply("Error in finding tweak. Are you sure you spelled it correctly?")
		...
		const embed = {
			color: "#0064FF",
			title: `Tweak Search for ${query} | Page ${page}`,
			thumbnail: {
				url: icon
			},
			fields: [
